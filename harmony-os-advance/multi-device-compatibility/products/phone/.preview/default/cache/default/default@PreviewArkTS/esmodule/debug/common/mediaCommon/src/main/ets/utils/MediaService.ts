...
import { SongConstants } from "@bundle:com.huawei.music.musichome/phone@constantsCommon/index";
import SongItemBuilder from "@bundle:com.huawei.music.musichome/phone@mediaCommon/ets/utils/SongItemBuilder";
import { AudioPlayerState, MusicPlayMode } from "@bundle:com.huawei.music.musichome/phone@mediaCommon/ets/viewmodel/MusicData";
import { SongItem } from "@bundle:com.huawei.music.musichome/phone@mediaCommon/ets/viewmodel/SongData";
import { PreferencesUtil } from "@bundle:com.huawei.music.musichome/phone@mediaCommon/ets/utils/PreferencesUtil";
import type { CardData } from '../viewmodel/CardData';
import { BackgroundUtil } from "@bundle:com.huawei.music.musichome/phone@mediaCommon/ets/utils/BackgroundUtil";
import { MediaTools } from "@bundle:com.huawei.music.musichome/phone@mediaCommon/ets/utils/MediaTools";
...
export class MediaService {
    ...
    async setAVMetadata() {
        ...
        try {
            if (this.context) {
                let mediaImage = await MediaTools.getPixelMapFromResource(this.context, this.songList[this.musicIndex].label as resourceManager.Resource);
    ...
    public async updateCardData() {
        try {
            ...
            if (this.musicIndex + SongConstants.ADD_INDEX_ONE === this.songList.length) {
                ...
                cardSongList = this.songList.slice(this.songList.length - SongConstants.SLICE_INDEX_TWO, this.songList.length);
                cardSongList.push(this.songList[0]);
            }
            else {
                cardSongList = this.songList.slice(this.musicIndex + SongConstants.SLICE_INDEX_ONE, this.musicIndex + SongConstants.SLICE_INDEX_FOUR);
            }
